...
export const fromAlbumDocument =
  (genre: Genre) =>
    ...
    pipe(
      apply.sequenceS(Validation.validation)({
        artist: lift("artist")(
          pipe(
            domHandler.document,
            domHandler.querySelectorTextContent("#name-section a"),
          ),
        ),
...
const parseTracks = (
...
): Validation<NonEmptyArray<Track>> => {
  ...
  return pipe(
    ...
    Either.chain(
      NonEmptyArray.traverseWithIndex(Validation.validation)(
        parseTrack(domHandler, name),
      ),
    ),
  );
};
...
const parseTrack =
  (domHandler: DomHandler, name: keyof AlbumMetadata) =>
  (i: number, tr: ParentNode): Validation<Track> =>
    pipe(
      apply.sequenceS(Validation.validation)({
        number: lift(`${name}[${i}].number`)(
          pipe(
            tr,
            domHandler.querySelectorTextContent(
              "td.track-number-col div.track_number",
            ),
            Either.chain((str) =>
              pipe(
                str.slice(0, -1),
