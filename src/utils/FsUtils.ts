...
export namespace FsUtils {
  ...
  export const exists = (f: FileOrDir): Future<boolean> =>
    pipe(stat(f), Future.map(Maybe.isSome));
  ...
  export const readdir = (dir: Dir): Future<List<FileOrDir>> =>
    pipe(
      Future.tryCatch(() =>
        fs.promises.readdir(dir.path, { withFileTypes: true })
      ),
  ...
  export const readFile = (file: File): Future<string> =>
    Future.tryCatch(() =>
      fs.promises.readFile(file.path, { encoding: "utf-8" })
    );
