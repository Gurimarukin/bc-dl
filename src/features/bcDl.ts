      ...
      pipe(
        ...
        NonEmptyArray.map(
          downloadAlbum(
            ...
            httpGetBuffer,
            execYoutubeDl,
          )(args.musicLibraryDir, args.genre),
        ),
...
const downloadAlbum =
  ...
  (url: Url): Future<void> =>
    pipe(
      ...
      Future.chainFirst(() =>
        Future.fromIOEither(logger.logWithUrl(url, "Fetching metadata")),
      ),
      Future.bind("metadata", () => getMetadata(httpGet)(genre, url)),

      Future.chainFirst(() =>
        Future.fromIOEither(logger.logWithUrl(url, "Downloading cover")),
      ),
      Future.bind("cover", ({ metadata }) =>
        downloadCover(httpGetBuffer)(metadata.coverUrl),
      ),

      Future.chainFirst(() =>
        Future.fromIOEither(logger.logWithUrl(url, "Downloading album")),
      ),
  ...
  pipe(
    ...
    Future.chain(
      flow(
        ...
        Either.mapLeft((e) =>
          Error(
            `Errors while listing mp3 files:\n${pipe(e, StringUtils.mkString("\n"))}`,
          ),
        ),
        Future.fromEither,
      ),
...
const getMp3File = (f: FileOrDir): Validation<File> => {
  if (FileOrDir.isDir(f))
    return Either.left(NonEmptyArray.of(`Unexpected directory: ${f.path}`));
  if (!isMp3File(f))
    return Either.left(NonEmptyArray.of(`Non mp3 file: ${f.path}`));
...
export const getActions = (
  ...
  const fromDeletions = getActionsFromDeletions(
    mp3Files,
    albumDir,
    metadata,
    cover,
  ...
  const cleanedAlbum = pipe(
    metadata.album,
    Album.stringify,
    StringUtils.cleanForCompare,
  );
  return pipe(
...
const getActionsFromDeletions =
  ...
  ): IO<NonEmptyArray<WriteTagsAction>> =>
    pipe(
...
const cleanFileNames =
  ...
  (unsortedDeletions: NonEmptyArray<string>): NonEmptyArray<CleanedFile> => {
    const cleanedFiles = pipe(
      mp3Files,
      NonEmptyArray.map((f) =>
        Tuple.of(f, StringUtils.cleanForCompare(f.basename)),
      ),
    );
    const deletions = pipe(
      unsortedDeletions,
      NonEmptyArray.sort(ordStringLength),
    );
    return pipe(
      deletions,
...
const getAction =
  ...
  ([file, cleanedName]: CleanedFile): IO<Validation<WriteTagsAction>> =>
    pipe(
      ...
      IO.map(
        Either.map((track) =>
          getWriteTagsAction(albumDir, metadata, cover, file, track),
        ),
      ),
    );
  ...
  Console.log(
    StringUtils.stripMargins(
      ...
      |${pipe(
        tracks,
        NonEmptyArray.map(
          (t) => `${logger.warnPrefix}- ${AlbumMetadata.Track.stringify(t)}`,
        ),
        StringUtils.mkString("\n"),
      )}`,
    ),
