...
const ensureAlbum =
  ...
  (url: Url): Future<void> =>
    pipe(
      ...
      Future.chainFirst(() => Future.fromIOEither(logger.logWithUrl(url, 'Ensuring files'))),
...
const getAction =
  (albumDir: Dir, metadata: AlbumMetadata, cover: Buffer, mp3Files: NonEmptyArray<FileWithTags>) =>
