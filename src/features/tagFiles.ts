  ...
  pipe(
    ...
    Future.chain((args) =>
      pipe(
        args.urls,
        NonEmptyArray.map(
          ensureAlbum(httpGet, httpGetBuffer)(args.musicLibraryDir, args.genre),
        ),
...
const ensureAlbum =
  (httpGet: HttpGet, httpGetBuffer: HttpGetBuffer) =>
  ...
  (url: Url): Future<void> =>
    pipe(
      ...
      Future.chainFirst(() =>
        Future.fromIOEither(logger.logWithUrl(url, "Ensuring files")),
      ),
...
const fileWithTagsNeaCodec: D.Decoder<
  List<FileWithRawTags>,
  NonEmptyArray<FileWithTags>
> = pipe(
  D.fromArray(fileWithTagsCodec),
  D.refine<List<FileWithTags>, NonEmptyArray<FileWithTags>>(
    List.isNonEmpty,
    "NonEmptyArray",
  ),
...
const getMp3Files = (
  musicLibraryDir: Dir,
): Future<NonEmptyArray<FileWithTags>> =>
  pipe(
...
const getActions = (
  ...
  pipe(
    metadata.tracks,
    NonEmptyArray.traverse(Validation.validation)(
    ...
    pipe(
      ...
      Either.map((filesAndTags) =>
        getWriteTagsAction(
          ...
          metadata,
          cover,
          NonEmptyArray.head(filesAndTags)[0],
          track,
        ),
      ),
    );
...
const trackMatchesTags =
  (metadata: AlbumMetadata, track: AlbumMetadata.Track) =>
  ([, tags]: FileWithTags): boolean =>
    (almostEquals(metadata.artist, tags.artist) ||
      almostEquals(metadata.artist, tags.performerInfo)) &&
    almostEquals(
      metadata.album.name,
      Album.fromRaw({ isTrack: false })(tags.album).name,
    ) &&
