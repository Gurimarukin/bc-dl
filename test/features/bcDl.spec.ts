...
describe("getActions", () => {
  const testGetActions =
    (metadata: AlbumMetadata, mp3Files: NonEmptyArray<File>) =>
    (
  ...
  const blackSabb: AlbumMetadata = {
    ...
    coverUrl: Url.wrap(
      "https://www.metal-archives.com/images/4/8/2/482.jpg?5008",
    ),
  };
    ...
    testGetActions(blackSabb, [
      ...
      expect(result).toStrictEqual(
        Either.right<Error, NonEmptyArray<WriteTagsAction>>([
          {
            file: File.fromPath(
              "black sabbath - black sabbath - black sabbath (1).mp3",
            ),
            newTags: getTags(blackSabb, imageBuffer, {
              number: 1,
              title: "Black Sabbath",
          ...
          {
            file: File.fromPath(
              "black sabbath - black sabbath - the wizard (2).mp3",
            ),
            newTags: getTags(blackSabb, imageBuffer, {
              number: 2,
              title: "The Wizard",
          ...
          {
            file: File.fromPath(
              "black sabbath - black sabbath - the wizard ii (5).mp3",
            ),
            newTags: getTags(blackSabb, imageBuffer, {
              number: 5,
              title: "The Wizard II",
        ...
        Either.right<Error, NonEmptyArray<WriteTagsAction>>([
          {
            ...
            newTags: getTags(albumArtistDifferent, imageBuffer, {
              number: 1,
              title: "Track",
            }),
  ...
  it("should Snakes Of Russia - Shallow End Diving", () =>
    testGetActions(snakesOfRussiaShallowEndDiving, [
      ...
      File.fromPath(
        "Snakes Of Russia - Shallow End Diving - Sombre Lux Remix.mp3",
      ),
    ])((result) => {
      expect(result).toStrictEqual(
        Either.right<Error, NonEmptyArray<WriteTagsAction>>([
          ...
          },
          {
            ...
            renameTo: File.fromPath(
              "02 - Shallow End Diving - Sombre Lux Remix.mp3",
            ),
          },
        ]),
      );
...
describe("ordStringLength", () => {
  it("should sort", () => {
    ...
    expect(
      pipe(["012", "looooong", "abc"], List.sort(ordStringLength)),
    ).toStrictEqual(expected);
    expect(
      pipe(["abc", "looooong", "012"], List.sort(ordStringLength)),
    ).toStrictEqual(expected);
  });
});
