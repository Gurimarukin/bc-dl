  ...
  pipe(
    ...
    Future.chain(
      flow(
        List.map((f) =>
          FileOrDir.isDir(f)
            ? FsUtils.rmdir(f, { recursive: true })
            : Future.unit,
        ),
...
export const httpGetMocked: HttpGet = (url) => {
  if (
    url === Url.wrap("https://inlustris.bandcamp.com/album/stella-splendens")
  ) {
...
const okResponseDocumentFromFile = (
  file: string,
