...
describe('StringUtils.cleanForCompare', () => {
  it('should clean 1', () => {
    ...
    const cleanedTrack = StringUtils.cleanForCompare('SET MIDI=SYNTH:1 MAP:G MODE:1')

    expect(cleanedFile).toStrictEqual('master boot record set midi=synth1 mapg mode1.mp3')
    expect(cleanedTrack).toStrictEqual('set midi=synth1 mapg mode1')
    expect(cleanedFile.includes(cleanedTrack)).toStrictEqual(true)
  })

  it('should clean 2', () => {
    const cleanedFile = StringUtils.cleanForCompare('MASTER BOOT RECORD - SET PATH=C -_METAL.mp3')
  ...
  it('should clean 3', () => {
    ...
    expect(cleanedFile).toStrictEqual('master boot record set sound=cclassical.mp3')
